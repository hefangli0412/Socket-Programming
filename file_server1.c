...
#define UDP1_FS "22000251"
#define TCP3 "41000251" // the port client will be connecting to in phase 3
#define HOSTNAME "nunki.usc.edu"
#define INFO "File_Server1 41000251"
...
int file_phase1()
{
	...
	printf("Phase 1: File Server 1 has UDP port number %s and IP address ", UDP1_FS);
...
int file_phase3()
{
	...
	printf("Phase 3: File Server 1 has TCP port %s and IP address ", TCP3);
	addr_list = (struct in_addr **)he->h_addr_list;
	for (i = 0; addr_list[i] != NULL; i++)
		printf("%s ", inet_ntoa(*addr_list[i]));
	printf(".\n");
	...
	while(1) {  // main accept() loop
		...
		if (!fork()) { // this is the child process
			...
			*/
			...
			printf("Phase 3: File Server 1 received the request from the <");
			for (i = 0; i < 7; i++)
			...
			*/
			...
			printf("Phase 3: File server1 has sent <");
			for (i = 0; i < 4; i++)
